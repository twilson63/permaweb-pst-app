...
<Modal {open} ok={false} width="w-11/12" maxWidth="max-w-5xl">
  ...
  <div class="flex space-x-8 flex-col md:flex-row space-y-8">
    ...
    <div class="flex md:w-1/2 flex-col">
      ...
      <div class="flex items-center font-mono my-8 justify-center">
        Price: {data.items.find((i) => i.type === "order")?.price / 1e6}
        <span class="ml-4">
          <svg
            width="35"
            height="35"
      ...
      <div class="flex justify-center">
        ...
        <div class="form-control w-1/3">
          <label class="label" for="percent">Price...</label>
          <div class="relative">
            <input
              id="percent"
              type="text"
      ...
      <div>
        ...
        <button class="btn btn-outline btn-block" on:click>Buy Asset</button>
      </div>
    </div>
  </div>
</Modal>
